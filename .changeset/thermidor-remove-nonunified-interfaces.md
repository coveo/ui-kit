...
Also pruned public type exports left behind by the removed controllers/actions and now unreachable from the retained surface: the `public/sort-types` sort-criterion types (`SortByRelevance`, `SortByDate`, `SortByField`, `SortByQRE`, `SortByNoSort`, `SearchSortCriterion`, `CommerceSortCriterion`, `SortCriterionFor`, `SortDirection`) and the cart mutation payloads (`SetCartItemsPayload`, `UpdateItemQuantityPayload`).
