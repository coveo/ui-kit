...
We need a stable top-level taxonomy that communicates how each workspace is primarily used without conflating architectural role with npm publication visibility.

## Decision Drivers

- A project's location should communicate its primary responsibility to contributors.
- The convention should distinguish reusable dependencies from runnable applications, validation suites, examples, repository automation, and source templates.
- Publication visibility must remain independent from architectural classification; private libraries are still packages.
- Package-specific supporting code should remain colocated unless it has an independent lifecycle or cross-project responsibility.
...
- **Pros:** Paths communicate intent; publication visibility remains orthogonal; each class can have appropriate ownership and CI behavior; the model accommodates future growth without a catch-all.
...
- **Cons:** Treats private reusable libraries as non-packages; mixes applications, tests, tools, services, and templates in a new catch-all; a visibility change could require moving an otherwise unchanged project.
...
| ------------ | ---------------------------------------------------------------------------------------------------- |
| `packages/`  | Reusable libraries, plugins, and distributed command-line tools consumed through a package interface |
| `apps/`      | Runnable applications, services, previews, and playgrounds                                           |
| `tests/`     | Independently operated, cross-project validation suites and test harnesses                           |
| `samples/`   | Consumer-facing examples that demonstrate supported public APIs                                      |
| `tools/`     | Repository automation operated by maintainers or CI                                                  |
...
- If consumers or other workspaces import it, install it, or invoke a distributed executable, it is a package.
- If maintainers start or deploy it as an application or service, it is an app.
- If its primary output is validation of other projects, it is a test workspace.
...
Option A addresses the underlying ambiguity rather than substituting publication status for architecture. A private mock library can remain in `packages/` because other workspaces consume its exports, while a private playground belongs in `apps/` because contributors run it interactively. This distinction remains stable if either project's publication metadata changes.
...
- **Positive:** Private reusable packages are no longer confused with applications or repository infrastructure.
- **Positive:** CI, ownership, and release policies can target project classes explicitly instead of relying on package privacy as a proxy.
...
- **Neutral:** This decision defines repository organization; it does not change package names, publication status, versioning, or dependency boundaries.
...
Keep reusable internal libraries such as `packages/platform-mock-api` under `packages/`. Evaluate ambiguous existing workspaces individually against the primary-interface rule instead of moving all private workspaces together.
