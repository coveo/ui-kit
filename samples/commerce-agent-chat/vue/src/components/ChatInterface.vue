...
<template>
  <main class="chat-shell">
    <section class="chat-container" aria-label="Commerce Agent Chat (Vue)">
      <header class="chat-header">
        <h1>Commerce Agent Chat (Vue)</h1>
      ...
      <section v-if="state.error" class="error-banner" role="alert">
        <p>{{ state.error }}</p>
        <button type="button" @click="dismissError">Dismiss</button>
      </section>

      <MessageInput :disabled="state.isLoading" @send="sendMessage" />
    </section>
  </main>
</template>
