...
export const AnswerGenerator = () => {
  ...
  const {updateQuery, executeSearch, logSearchboxSubmit, rgaController} = useMemo(() => {
    ...
    const {updateQuery}: QueryActionCreators = loadQueryActions(headlessEngine); // callout[Refer to <a href="https://docs.coveo.com/en/headless/latest/reference/functions/Search.loadQueryActions.html>`loadQueryActions`</a> for additional details.]
    const {executeSearch}: SearchActionCreators = loadSearchActions(headlessEngine); // callout[Refer to <a href="https://docs.coveo.com/en/headless/latest/reference/functions/Search.loadSearchActions.html">`loadSearchActions`</a> for additional details.]
  ...
  useEffect(() => {
    const unsubscribe = rgaController.subscribe(() => setRgaState(rgaController.state));
