...
import {PRODUCT_LIST_SCHEMA_ID} from '../components.js';
...
interface ProductCarouselProps {
  controllers: {
    productListController: {
      controllerId: string;
      controllerSchema: typeof PRODUCT_LIST_SCHEMA_ID;
    };
  };
  heading?: string;
}
  ...
  return (
    <section className={styles.container}>
      ...
      <div className={styles.trackWrapper}>
        {canScrollLeft && (
          <button className={`${styles.navButton} ${styles.navLeft}`} onClick={() => scrollBy('left')} aria-label="Scroll left">‹</button>
        )}
        <div className={styles.track} ref={trackRef} tabIndex={0} role="region" aria-label={heading}>
          {products.map((product: Product) => (
            <div className={styles.cardSlot} key={product.permanentid}>
              <A2UIProductCard ec_name={product.ec_name} ec_brand={product.ec_brand} ec_price={product.ec_promo_price ?? product.ec_price} ec_image={product.ec_images?.[0]} ec_product_id={product.permanentid} />
            </div>
          ))}
        </div>
        {canScrollRight && (
          <button className={`${styles.navButton} ${styles.navRight}`} onClick={() => scrollBy('right')} aria-label="Scroll right">›</button>
        )}
      </div>
    </section>
  );
