import type {ComparisonProduct, ComparisonAttribute} from '@coveo/thermidor-schema';
import {useAdvertisedController} from '../controllers.js';
import {useStateSource} from '../state-source-context.js';
import {COMPARISON_TABLE_SCHEMA_ID} from '../components.js';
...
interface ComparisonTableProps {
  controllers: {
    comparisonTableController: {
      controllerId: string;
      controllerSchema: typeof COMPARISON_TABLE_SCHEMA_ID;
    };
  };
}
...
export function ComparisonTableRenderer({props}: {props: ComparisonTableProps}) {
  ...
  const controller = useAdvertisedController(stateSource, props.controllers.comparisonTableController);
    ...
    <>
      <section className={styles.container}>
        ...
        <div className={styles.tableWrapper}>
          <div className={styles.table} role="table" aria-label="Cold-Water Surfing Wetsuits Comparison">
            <div className={styles.row} role="row">
              <div className={styles.labelCell} role="columnheader">Product</div>
              {products.map((p: ComparisonProduct) => (
                <div key={p.productId} className={styles.productCell}>
                  {p.imageUrl && <img className={styles.productImage} src={p.imageUrl} alt={p.name} loading="lazy" />}
                  <span className={styles.productName}>{p.name}</span>
                </div>
              ))}
            </div>
            <div className={styles.row} role="row">
              <div className={styles.labelCell} role="rowheader">Price</div>
              {products.map((p: ComparisonProduct) => (
                <div key={`price-${p.productId}`} className={styles.valueCell} role="cell">
                  <span className={styles.priceValue}>{p.price !== undefined ? formatPrice(p.price) : '—'}</span>
                </div>
              ))}
            </div>
            {attributes.filter((attr: ComparisonAttribute) => attr.key !== 'brand').map((attr: ComparisonAttribute) => (
              <div key={attr.key} className={styles.row} role="row">
                <div className={styles.labelCell} role="rowheader">{attr.label}</div>
                {products.map((p: ComparisonProduct) => (
                  <div key={`${attr.key}-${p.productId}`} className={styles.valueCell} role="cell">{p.values[attr.key] ?? '—'}</div>
                ))}
              </div>
            ))}
          </div>
