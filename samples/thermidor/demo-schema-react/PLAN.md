...
> **Note (current model).** This is a historical planning document for the sibling `demo-react` sample and describes an earlier controller-based approach (`ConverseController`, `useRemoteController`, AG-UI `StateSnapshot`). The `demo-schema-react` sample that hosts this file now follows the **inline component-state model**: a single `id`/`component` identity, A2-UI Data_Binding objects `{ "path": <JSON Pointer> }`, `/state/<id>` `updateDataModel` transport (no AG-UI `StateSnapshot`), named-slot container composition, dumb renderers with no controller join, and a single `session.dispatchAction` dispatch entry point over the HTTP Action_Channel. Client-side Zod validation is retained (transparently, inside `@coveo/thermidor`); displayed-value formatting is agent-produced; A2-UI is the only supported source of component state; and the sample intentionally diverges from A2-UI bidirectional input binding (in-progress input stays in local React state). See `README.md`, `IMPLEMENTATION.md`, and ADR-011 in `@coveo/thermidor-schema` for the authoritative model. The controller-based description below is superseded and kept only as historical planning context.
