...
| `src/a2ui/surfaces.tsx`                                  | Extracts A2-UI messages from activities and converts v1.0 → v0.9, carrying each `{ "path": ... }` binding through byte-for-byte and passing `updateDataModel` ops to the catalog resolver                                                                                                                                                           |
...
AgentResponseBlock
  ...
  └── ThermidorA2UISurfaces
        ↓ convertV1ToV09 adapter (carries {path} bindings byte-for-byte)
