...
import {Catalog, type ComponentApi, createFunctionImplementation} from '@a2ui/web_core/v0_9';
...
export const thermidorCatalogDefinitions = {
  ...
  Cart: {
    name: 'Cart',
    schema: cartPropsSchema,
  },
} satisfies Record<string, ComponentApi>;

const ProductCarousel = createComponentImplementation(
  thermidorCatalogDefinitions.ProductCarousel,
  ({props}) => {
            ...
            return (
              <article className="product-card" key={product.permanentid}>
                ...
                <div className="product-meta">
                  ...
                  <span>{product.ec_rating == null ? 'Not rated' : `★ ${product.ec_rating}`}</span>
                </div>
                <small>
...
const Cart = createComponentImplementation(thermidorCatalogDefinitions.Cart, ({props}) => {
...
export function createThermidorCatalog(
  ...
  return new Catalog(
    ...
    [...basicCatalog.components.values(), ProductCarousel, Cart],
    [...basicCatalog.functions.values(), dispatchControllerAction],
