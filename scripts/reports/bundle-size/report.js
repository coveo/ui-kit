...
function buildVisualReport(rows) {
  ...
  const rowsCombinedAcrossBundles = rows.reduce(
    (accumulated, current) => accumulated.concat(current),
    []
  );
  const rowsWithColumnsConcatenated = rowsCombinedAcrossBundles.map(
    (row) => '|' + row.join('|')
  );
