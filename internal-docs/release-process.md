...
Scheduled releases will commit the new `pnpm-lock.yaml` to the main branch, which ensures that contributors can run `pnpm install` without unnecessary changes being made to their `pnpm-lock.yaml`.
