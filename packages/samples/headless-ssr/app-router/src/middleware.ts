...
/**
 * This will assign users a new client ID on their first visit, unless they have opted out of tracking.
 * Make sure to return the response object from the middleware function.
 *
 * For more on middlewares, see https://nextjs.org/docs/app/building-your-application/routing/middleware.
 */
...
/**
 * Middlewares run by default for every request, including the ones for getting assets like JavaScript, CSS, and image files.
 * We use the `matcher` property to ensure that the AnalyticsMiddleware only runs on the page load.
