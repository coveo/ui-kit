...
export type MethodsFromController<T extends Controller> = Omit<
  T,
  'subscribe' | 'state'
>;
...
export interface UseControllerReturnType<T extends Controller> {
  state: T['state'];
  methods: MethodsFromController<T>;
}
...
function buildController(
  ...
  return {
    methods: bindMethods(methods, headlessController),
...
export function useController(
  ...
  const {methods, getStateSnapshot, onStateChanged} = useDeepCompareMemo(
...
export type UseController = typeof useController;
