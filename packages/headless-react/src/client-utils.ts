 ...
 */
export function useSyncMemoizedStore<T>(subscribe: Subscriber, getSnapshot: SnapshotGetter<T>): T {
  ...
  if (useHasDepsChanged([subscribe, getSnapshot]) || snapshot.current === null) {
    snapshot.current = getSnapshot();
  }
