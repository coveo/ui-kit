...
type AtomicCommerceInterfaceProps = React.ComponentProps<typeof AtomicCommerceInterface>;
...
export const InterfaceWrapper = (props: React.PropsWithChildren<WrapperProps>) => {
  ...
  const interfaceRef = useRef<React.ElementRef<typeof AtomicCommerceInterface>>(null);
  ...
  useEffect(() => {
    ...
    if (!initialization) {
      ...
      initialization.then(() => {
        ...
        onReady(commerceInterfaceAtomic.executeFirstRequest.bind(commerceInterfaceAtomic));
      });
    }
