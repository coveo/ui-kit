...
This article guides you through the process of integrating [Coveo RGA](https://docs.coveo.com/en/n9de0370/) in a front-end app with the [`@coveo/headless`](./index.html) library.
...
First, use [`buildSearchEngine`](../functions/Search.buildSearchEngine.html) to instantiate the [`SearchEngine`](../interfaces/Search.SearchEngine.html) that will be used to create your RGA controller.
...
You can optionally provide `buildGeneratedAnswer` with additional configuration options, as defined by [`GeneratedAnswerProps`](../interfaces/Search.GeneratedAnswerProps.html), to enhance the relevance of the response generated.

The engine is what receives dispatched actions, not the RGA controller ([`GeneratedAnswer`](../interfaces/Search.GeneratedAnswer.html)).
...
In the following example a [`SearchEngine`](../interfaces/Search.SearchEngine.html) is created.
The [`GeneratedAnswer`](../interfaces/Search.GeneratedAnswer.html) controller is created with that engine.
...
The `@coveo/headless` exports functions to create a common set of action creators, you can [extend these with your own custom set](./usage/extend-controllers.html).
...
These sources, called citations, are available through the [`citations`](../interfaces/Search.GeneratedAnswerState.html#citations) field of the [`GeneratedAnswerState`](../interfaces/Search.GeneratedAnswerState.html).

Each citation follows the [`GeneratedAnswerCitation`](../interfaces/Search.GeneratedAnswerCitation.html) interface, which provides useful metadata such as the source title and URI.
...
Use [`buildInteractiveCitation`](../functions/Search.buildInteractiveCitation.html) to track events on a specific citation.
...
Following is an example of rendering a [`GeneratedAnswerCitation`](../interfaces/Search.GeneratedAnswerCitation.html) that implements event tracking.
