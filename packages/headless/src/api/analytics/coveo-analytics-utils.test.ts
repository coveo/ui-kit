...
import type {PlatformRequestOptions, PreprocessRequest} from '../preprocess-request.js';
...
describe('coveo-analytics-utils', () => {
  it('should properly wrap preprocessRequest', () => {
    ...
    const wrapped = wrapPreprocessRequest(pino({level: 'silent'}), preprocessRequestThatThrows);
    const output = wrapped!({url: 'foo'}, 'analyticsFetch') as NonNullable<PlatformRequestOptions>;

    expect(output.url).toBe('foo');
  });

  it('should properly wrap analyticsClientSendEventHook', () => {
    const analyticsClientSendEventHookThatThrows: AnalyticsClientSendEventHook = (_, payload) => {
