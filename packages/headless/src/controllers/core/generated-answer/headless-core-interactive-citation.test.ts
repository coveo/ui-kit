...
describe('InteractiveCitation', () => {
  ...
  function initializeInteractiveCitation(delay?: number, answerId?: string) {
    ...
    interactiveCitation = buildInteractiveCitationCore(engine, analyticsClient, {
  ...
  it('when calling select(), calls logCitationOpen with citation id', () => {
    ...
    expect(analyticsClient.logCitationOpen).toHaveBeenCalledWith(mockCitation.id, undefined);
  });
  ...
  it('when an answerId is provided, passes it to logCitationOpen', () => {
    ...
    expect(analyticsClient.logCitationOpen).toHaveBeenCalledWith(mockCitation.id, 'answer-id');
  });
