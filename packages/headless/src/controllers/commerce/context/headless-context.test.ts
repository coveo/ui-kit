...
describe('headless commerce context', () => {
  ...
  it('setLanguage dispatches #setContext', () => {
    ...
    expect(setContext).toHaveBeenCalledWith(expect.objectContaining({language: 'new-language'}));
  });

  it('setCountry dispatches #setContext', () => {
    context.setCountry('new-country');
    expect(setContext).toHaveBeenCalledWith(expect.objectContaining({country: 'new-country'}));
  });

  it('setCurrency dispatches #setContext', () => {
    context.setCurrency('CAD');
    expect(setContext).toHaveBeenCalledWith(expect.objectContaining({currency: 'CAD'}));
  });
