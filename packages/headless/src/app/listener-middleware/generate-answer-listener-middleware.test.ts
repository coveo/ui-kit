...
vi.mock('../../features/follow-up-answers/follow-up-answers-actions.js', () => ({
...
vi.mock('../../api/knowledge/answer-generation/agents/answer-agent/answer-agent-runner.js', () => ({
    ...
    describe('when agentId is set', () => {
        ...
        beforeEach(() => {
          store = configureStore({
            reducer: {
              ...
              answerGenerationApi: () => ({}),
              configuration: (state = {knowledge: {agentId: 'some-agent-id'}}) => state,
              query: (state = {q: ''}) => state,
            },
      ...
      describe('when query is not empty', () => {
        ...
        it('should abort current run, dispatch head answer reset and follow-up reset before running the answer agent', async () => {
          ...
          const abortCallOrder = answerRunnerAbortRun.mock.invocationCallOrder[0];
          // oxlint-disable-next-line @typescript-eslint/no-explicit-any -- unit tests
          const resetCallOrder = (resetAnswer as any).mock.invocationCallOrder[0];
          // oxlint-disable-next-line @typescript-eslint/no-explicit-any -- unit tests
          const resetFollowUpCallOrder = (resetFollowUpAnswers as any).mock.invocationCallOrder[0];
