...
const DEFAULT_URL = new URL('../icons/', import.meta.url).href;
...
export class DarkModeToggle extends HTMLElement {
  ...
  _updateAppearance() {
    // Hide or show the light-related features dependent on the appearance,
  ...
  _updateCheckbox() {
    if (this.mode === LIGHT) {
      this._checkboxLabel.style.setProperty(
        `--${NAME}-checkbox-icon`,
        `var(--${NAME}-light-icon,url("${DEFAULT_URL}sun.svg"))`
      );
      ...
      this._checkboxLabel.style.setProperty(
        `--${NAME}-checkbox-icon`,
        `var(--${NAME}-dark-icon,url("${DEFAULT_URL}moon-and-stars.svg"))`
      );
