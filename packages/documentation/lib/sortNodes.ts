...
export const applyTopLevelOrderingNode = (root: NavNode, order: string[]) => {
...
const applyOrderingArray = (items: NavNode[], order: string[]) => {
  const spec = order.map((s) => normalize(s));
  const wildcard = spec.indexOf('*');
  items.sort((a, b) => rankCompare(a, b, spec, wildcard));
};
...
const alphaByText = (a: NavNode, b: NavNode) => {
