...
it('projects a crash report through the real Sentry SDK without network access', async () => {
  ...
  try {
    ...
    // The Sentry Node SDK overwrites `contexts.os` and `contexts.runtime` with the
    // live runtime of the machine running `report` (e.g. `runtime.version` becomes
    // `process.version`), so they are environment-dependent and differ on CI. The
    // crash's own os/arch/node are asserted deterministically via `tags` above;
    // `device` and `Custom` are projected verbatim by submit-report from the report.
