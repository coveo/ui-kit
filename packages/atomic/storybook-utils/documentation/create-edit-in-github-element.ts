type EditButtonElement = HTMLDivElement & {
  href?: string;
  _anchor: HTMLAnchorElement;
};

export function createEditInGithubElement(): EditButtonElement {
  const wrapper = document.createElement('div') as EditButtonElement;
  // use CSS classes; container receives placement styles from stylesheet
  wrapper.className = 'atomic-edit-github-container';
  ...
  // anchor styling is provided by stylesheet
  anchor.className = 'atomic-edit-github-link';
  ...
  label.className = 'atomic-edit-github-label';
  label.textContent = 'Edit in GitHub';

  // provide minimal inline styles to avoid initial browser-link flash before CSS loads
  anchor.style.color = 'rgba(0,0,0,0.72)';
  anchor.style.backgroundColor = '#e5e8e8';

  // purely CSS-driven hover/focus behavior — no JS listeners required
