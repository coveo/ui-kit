...
describe('TextArea Search Box Test Suites', () => {
  describe('with maxWithoutQuery capping suggestions & recentQueries', () => {
    ...
    describe('with input', () => {
      ...
      describe('after selecting a suggestion with the mouse focuses search box with recent query', () => {
        ...
        SearchBoxAssertions.assertFocusSearchBox(SearchBoxSelectors.textArea);
        SearchBoxAssertions.assertHasText(
          'Recent query 1',
          SearchBoxSelectors.textArea
        );
  ...
  describe('with minimum query length to enable search', () => {
    ...
    it('search button is enabled when a query with minimum length is input', () => {
      typeSearchTextArea(testQuery.slice(0, minimumQueryLength - 1)); // enter query less than min len
      SearchBoxSelectors.submitButton().should('be.disabled');
      SearchBoxAssertions.assertNoSuggestionGenerated();

      typeSearchTextArea(testQuery.slice(minimumQueryLength - 1)); // enter rest of the query
      SearchBoxSelectors.submitButton().should('not.be.disabled');

      typeSearchTextArea('{downarrow}'.repeat(numOfSuggestions));
    ...
    it('search button is disabled when query is deleted', () => {
      ...
      typeSearchTextArea('{backspace}'.repeat(minimumQueryLength), '');
      SearchBoxSelectors.submitButton().should('be.disabled');
    });
  });
  ...
  describe('with the query syntax enabled', () => {
    beforeEach(() => {
      new TestFixture()
        .with(addTextAreaSearchBox({props: {'enable-query-syntax': ''}}))
        .with(
          addResultList(
            buildTemplateWithoutSections(
