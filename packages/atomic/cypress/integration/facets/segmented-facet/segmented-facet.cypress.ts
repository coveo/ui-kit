...
describe('Segmented Facet Test Suites', () => {
  ...
  describe.skip('with depends-on', () => {
    describe('as a dependent & parent', () => {
      ...
      before(() => {
        new TestFixture()
          ...
          .with(
            addSegmentedFacet({'facet-id': parentFacetId, field: parentField})
          )
          .init();
      });

      CommonFacetAssertions.assertDisplayFacet(
        SegmentedFacetSelectors.withId(facetId),
        true
      );
      CommonFacetAssertions.assertDisplayFacet(
        SegmentedFacetSelectors.withId(parentFacetId),
        true
      );
    ...
    describe('with two dependencies', () => {
      before(() => {
        new TestFixture()
          .with(addSegmentedFacet({'facet-id': 'abc', field: 'objecttype'}))
          .with(addSegmentedFacet({'facet-id': 'def', field: 'filetype'}))
          .with(
            addSegmentedFacet({
              'facet-id': 'ghi',
              field,
      ...
      CommonAssertions.assertContainsComponentError(
        SegmentedFacetSelectors.withId('ghi'),
        true
      );
    });
  });
