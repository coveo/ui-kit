...
const configs: Array<
  [
    ...
    string,
    (template?: HTMLElement | undefined) => (fixture: TestFixture) => void,
    string,
    boolean
  ]
> = [foldedResultListConfig, resultListConfig];

configs.forEach(
  ([componentSelectors, componentTag, addResultFn, title, isFolded]) => {
    describe(title, () => {
      it('should load', () => {
