...
import {Rating} from '../../search/atomic-rating/atomic-rating';
 ...
 * An `atomic-product-listing-rating-range-facet` displays a facet of the results for the current query as ratings.
 ...
 */
@Component({
  tag: 'atomic-product-listing-product-rating-range-facet',
  styleUrl: 'atomic-product-listing-product-rating-range-facet.pcss',
  shadow: true,
})
    ...
    BaseFacet<ProductListingNumericFacet>
{
   ...
   */
  @Prop({reflect: true}) public shouldRender = true;
  /**
  ...
  public render() {
    if (this.error || !this.facet.state.enabled) {
