...
test.describe('atomic-result-fields-list', () => {
  ...
  test('should render all fields with their values', async ({resultFieldsList, page}) => {
    ...
    await expect(resultFieldsList.atomicText.filter({hasText: 'John Doe'})).toBeVisible();

    await expect(page.getByText('Source:')).toBeVisible();
    await expect(resultFieldsList.atomicText.filter({hasText: 'Documentation'})).toBeVisible();

    await expect(page.getByText('FileType:')).toBeVisible();
    await expect(resultFieldsList.atomicText.filter({hasText: 'pdf'})).toBeVisible();
  });
});
