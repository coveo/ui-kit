...
- **Feedback Collection**: Built-in like/dislike buttons to collect user feedback on a specific answer
...
When an `agent-id` is set, the component:
...
- **`agent-id`**: The unique identifier of the Coveo AI agent to use for answer generation. When set, the component uses the Agent API instead of the Answer API and enables multi-turn follow-up questions.

> **Note:** The `agent-id` and `answer-configuration-id` properties are mutually exclusive. Use `agent-id` for the conversational experience, or `answer-configuration-id` for single-turn answer generation via the Answer API. If neither is provided, the component falls back to the Search API for generated answers.

> **Note:** The `collapsible` property is not supported when the conversational experience is enabled. The threaded conversation layout manages its own content display.
