...
This can offer a powerful navigational experience, quickly leading a user to relevant results.
  ...
  - This component uses `;` to separate individual values in the multi-value field, and the character defined by the `delimiting-character` attribute to separate hierarchical levels within each value.
    We recommend setting `delimiting-character="|"` so that `;` continues to separate individual values, while `|` separates hierarchical levels within each value.
