...
test.describe('atomic-result-link', () => {
  ...
  test('should have href attribute set to result clickUri', async ({resultLink}) => {
    const anchor = resultLink.anchor().first();
    await expect(anchor).toHaveAttribute('href');
    await expect(anchor).toBeVisible();
  });

  test('should render as links and be clickable', async ({resultLink, page}) => {
  ...
  test('should render atomic-result-text when no default slot provided', async ({resultLink}) => {
