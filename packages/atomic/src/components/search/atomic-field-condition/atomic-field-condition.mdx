...
You can specify multiple `must-match-*` and `must-not-match-*` attributes, each targeting a different field. Each attribute accepts multiple comma-separated values.

**Comma represents OR:** Within a single attribute, comma-separated values represent a logical OR. For example, `must-match-filetype="pdf,docx"` means the filetype must be pdf OR docx.

**Multiple attributes represent AND:** All conditions from different attributes must be met (logical AND) for content to render.

If you set both `must-match-*` and `must-not-match-*` for the same field with overlapping values, the condition will never be satisfied.
