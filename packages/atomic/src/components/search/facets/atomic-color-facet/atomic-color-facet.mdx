...
<AtomicDocTemplate
  ...
  githubPath="search/facets/atomic-color-facet/atomic-color-facet.ts"
  tagName="atomic-color-facet"
  className="AtomicColorFacet"
>

The `atomic-color-facet` component displays a facet of the results for the current query as colors. This is particularly useful when you want to represent facet values visually, such as file types, categories, or any field that can be associated with colors or icons.
...
- **box** (default): Displays facet values as colored boxes with labels below
- **checkbox**: Displays facet values as checkboxes with colored indicators

```html
<!-- Box display mode (default) -->
<atomic-color-facet field="filetype" label="File Types" display-values-as="box"></atomic-color-facet>

<!-- Checkbox display mode -->
<atomic-color-facet field="filetype" label="File Types" display-values-as="checkbox"></atomic-color-facet>
```
