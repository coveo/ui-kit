...
- The button appears when the first search has been executed and results are available
- The button shows a placeholder while the first search is executing
- The button does not appear if there's an error or no results
- Clicking the button opens a modal that covers the viewport
- The modal can be closed by clicking the close button, the "View Results" button, or pressing the Escape key
