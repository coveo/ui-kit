...
describe('atomic-field-condition', () => {
  ...
  const renderAtomicFieldCondition = async ({
    props = {},
    slottedContent,
    ...
    const {element} = await renderInAtomicSearchInterface<AtomicFieldCondition>(
      {
        template: html`<atomic-field-condition
    ...
    return {
      ...
      parts: (element: AtomicFieldCondition) => {
  ...
  describe('#initialize', () => {
    it('should not set the error when using the default props', async () => {
      const {element} = await renderAtomicFieldCondition();
  ...
  describe('when the "atomic/redden" event is dispatched', () => {
    ...
    it('should add the "text-red-600" class to the message paragraph', async () => {
      const {element, parts} = await renderAtomicFieldCondition();
  ...
  describe('when rendering (#render)', () => {
    ...
    it('should render the button with the correct text when the query is revealed', async () => {
      const {element, parts, toggleRevealQueryButton} =
        await renderAtomicFieldCondition();
