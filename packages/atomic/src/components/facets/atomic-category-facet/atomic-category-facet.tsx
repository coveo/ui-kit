  ...
  implements InitializableComponent, BaseFacet<CategoryFacet>
{
  ...
  private renderValuesTree(
    ...
    startWithAllCategories: boolean
  ) {
    if (!this.hasParents) {
      return;
    }

    if (startWithAllCategories) {
      return (
        <li class="contents">
    ...
    return (
      <FacetValueLink
        ...
        numberOfResults={parents[0].numberOfResults}
        isSelected={true}
        i18n={this.bindings.i18n}
        onClick={() => {
          this.activeValueFocus.focusAfterSearch();
  ...
  private renderChild(facetValue: CategoryFacetValue) {
    const displayValue = getFieldValueCaption(
      this.field,
      facetValue.value,
      this.bindings.i18n
  ...
  private renderChildren() {
    ...
    return (
      <ul part="values" class={this.hasParents ? 'pl-2' : 'mt-3'}>
        {this.facetState.values.map((value) => this.renderChild(value))}
      </ul>
    );
  }
