 ...
 */
@customElement('atomic-recs-result')
@withTailwindStyles
export class AtomicRecsResult extends ChildrenUpdateCompleteMixin(LitElement) {
  ...
  /**
   * Whether `atomic-result-link` components nested in the `atomic-recs-result` should stop click event propagation.
   */
  @property({
   ...
   * The result item.
   */
  @property({type: Object}) result!: RecsResult;
   ...
   * Global Atomic state.
   */
  @property({type: Object}) store?: RecsStore;
  ...
  constructor() {
    ...
    new ValidatePropsController(
      ...
      AtomicRecsResult.propsSchema
    );
  }

  public resolveResult = (event: CustomEvent<(result: RecsResult) => void>) => {
  ...
  public resolveInteractiveResult = (
    event: CustomEvent<(interactiveResult: RecsInteractiveResult) => void>
  ) => {
    event.preventDefault();
    event.stopPropagation();
  ...
  public resolveResultDisplayConfig = (event: CustomEvent<(config: DisplayConfig) => void>) => {
    event.preventDefault();
    event.stopPropagation();
    event.detail({
      density: this.density,
  ...
  public async connectedCallback() {
    ...
    this.itemLayoutController = new ItemLayoutController(this, {
      elementPrefix: 'atomic-recs-result',
      renderingFunction: () => this.renderingFunction,
