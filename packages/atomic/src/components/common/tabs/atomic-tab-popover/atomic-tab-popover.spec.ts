...
describe('atomic-tab-popover', () => {
  ...
  const renderAtomicTabPopover = async ({
    props = {},
    slottedContent,
    ...
    return {
      ...
      parts: (element: AtomicTabPopover) => {
  ...
  describe('#initialize', () => {
    it('should not set the error when using the default props', async () => {
      const {element} = await renderAtomicTabPopover();
  ...
  describe('when the "atomic/redden" event is dispatched', () => {
    ...
    it('should add the "text-red-600" class to the message paragraph', async () => {
      const {element, parts} = await renderAtomicTabPopover();
