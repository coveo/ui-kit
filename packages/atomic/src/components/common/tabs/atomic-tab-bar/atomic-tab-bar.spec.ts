...
describe('atomic-tab-bar', () => {
  ...
  const renderAtomicTabBar = async ({
    props = {},
    slottedContent,
    ...
    const {element} = await renderInAtomicSearchInterface<AtomicTabBar>({
      template: html`<atomic-tab-bar
        entity-to-greet=${ifDefined(props.entityToGreet)}
        ?is-vulcan=${props.isVulcan}
      >${ifDefined(slottedContent)}
      </atomic-tab-bar>`,
      selector: 'atomic-tab-bar',
      bindings: (bindings) => {
  ...
  describe('#initialize', () => {
    it('should not set the error when using the default props', async () => {
      const {element} = await renderAtomicTabBar();
  ...
  describe('when the "atomic/redden" event is dispatched', () => {
    ...
    it('should add the "text-red-600" class to the message paragraph', async () => {
      const {element, parts} = await renderAtomicTabBar();
