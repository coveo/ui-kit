...
type ItemLinkEventProps = Pick<
  ItemLinkProps,
  'onSelect' | 'onBeginDelayedSelect' | 'onCancelPendingSelect' | 'stopPropagation'
>;
...
export const bindAnalyticsToLink = (
  ...
  const touchStartHandler = (e: Event) => stopPropagationAndProcess(e, onBeginDelayedSelect);
  const touchEndHandler = (e: Event) => stopPropagationAndProcess(e, onCancelPendingSelect);

  (['click', 'contextmenu', 'mousedown', 'mouseup'] as const).forEach((eventName) =>
