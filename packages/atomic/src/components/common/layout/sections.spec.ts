  ...
  describe('isResultSectionNode', () => {
    describe('with result section elements', () => {
      ...
      it.each(resultSectionTags)(
        ...
        (tagName) => {
          const element = document.createElement(tagName);
          // Simulate uppercase tag name (though browsers normalize to lowercase)
  ...
  describe('containsSections', () => {
    ...
    describe('with HTMLCollection content', () => {
      ...
      it('should handle nested elements correctly', () => {
        ...
        // HTMLCollection only includes direct children, so nested section won't be detected
        expect(containsSections(container.children)).toBe(false);

        // But if we check the nested container's children, it should be detected
    ...
    describe('edge cases', () => {
      ...
      it('should handle section tags with attributes in string', () => {
        const content =
          '<atomic-product-section-name class="highlight" id="name">Content</atomic-product-section-name>';
        expect(containsSections(content)).toBe(true);
      });

      it('should be case sensitive for string content', () => {
        const content =
          '<ATOMIC-RESULT-SECTION-VISUAL>Content</ATOMIC-RESULT-SECTION-VISUAL>';
        expect(containsSections(content)).toBe(false);
      });
