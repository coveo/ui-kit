...
## Usage
This component is used to define your template for how products are displayed in the UI. 
It can be used inside an `atomic-commerce-product-list`, `atomic-commerce-recommendation-list`, or `atomic-search-box-instant-products` and it must have a `<template>` as it's child.

An example using the `atomic-commerce-product-list`:
```html
