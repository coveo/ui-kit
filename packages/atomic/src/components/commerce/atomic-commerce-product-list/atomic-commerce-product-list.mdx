...
When [displaying products as a list or grid](https://static.cloud.coveo.com/atomic/v3/storybook/index.html?path=/docs/guides-display-results-display-results-as-a-list-or-grid--docs), the component applies a basic template by default. You can also [define custom templates](https://static.cloud.coveo.com/atomic/v3/storybook/index.html?path=/docs/guides-display-results-introduction--docs#defining-a-result-template).
...
When [displaying products as a table](https://static.cloud.coveo.com/atomic/v3/storybook/index.html?path=/docs/guides-display-results-display-results-as-a-table--docs), there is no default template. You must define a template that includes at least one `atomic-table-element` component, which serve as columns.
...
### Displaying a specific product listing
If you want `atomic-commerce-product-list` to display a specific product listing, you will need to ensure that the `atomic-commerce-interface` at
level component of the hierarchy for this interface has its type set to `product-listing`.

You will also need to ensure that configuration being used to initialize the interface has the `view` url of the configuration
[`context`](https://docs.coveo.com/en/headless/latest/reference/interfaces/Commerce.ContextOptions.html) set to a configured listing page.
