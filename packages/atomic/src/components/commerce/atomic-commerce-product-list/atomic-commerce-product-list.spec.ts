...
describe('atomic-commerce-product-list', () => {
  ...
  describe("when interface element type is 'product-listing'", () => {
    it('should initialize #searchOrListing as product listing controller', async () => {
      ...
      const productListingController = vi.mocked(buildProductListing).mock.results[0].value;
    ...
    it('should initialize #summary as product listing summary controller', async () => {
      ...
      const summaryController =
        vi.mocked(buildProductListing).mock.results[0].value.summary.mock.results[0].value;
    ...
    it('should subscribe to summary controller state changes', async () => {
      ...
      summary.mockReturnValue(buildFakeSummary({implementation: {subscribe: summarySubscribeSpy}}));
  ...
  describe("when interface element type is 'search'", () => {
    ...
    it('should initialize #summary as search summary controller', async () => {
      ...
      const summaryController = buildSearchSpy.mock.results[0].value.summary.mock.results[0].value;
  ...
  it("should add 'atomic/selectChildProduct' event listener with correct callback", async () => {
    ...
    expect(element.searchOrListing.promoteChildToParent).toHaveBeenCalledWith('childProductId');
  });
  ...
  describe('#updated', () => {
    ...
    it.each([
      ...
      {
        description: 'should not change #isEveryProductReady when staying in loading state',
        oldState: true,
        newState: true,
        expectedResult: true,
      },
      {
        description: 'should not change #isEveryProductReady when staying in not loading state',
        oldState: false,
        newState: false,
        expectedResult: true,
      },
      ...
      element.updated(new Map([['searchOrListingState', {isLoading: oldState}]]));
  ...
  describe.each<{display: ItemDisplayLayout}>([{display: 'grid'}, {display: 'list'}])(
    ...
    ({display}) => {
      it('should not set the renderingFunction on the atomic product itself', async () => {
        ...
        const atomicProductElement = element.shadowRoot?.querySelector('atomic-product');

        expect(atomicProductElement?.renderingFunction).toBe(mockRenderingFunction);
      });
  ...
  const renderAtomicProductTestCases = (display: ItemDisplayLayout) => {
    it('should render correct # of atomic-product', async () => {
      ...
      const atomicProductElements = element.shadowRoot?.querySelectorAll('atomic-product');
    ...
    describe('when rendering atomic-product', () => {
      if (display === 'table') {
        it('should pass 1st product template to #content', async () => {
          ...
          expect(atomicProductElements?.[0].content?.querySelector('div')?.textContent).toBe(
            'Hello from 123'
          );
          expect(atomicProductElements?.[1].content?.querySelector('div')?.textContent).toBe(
            'Hello from 123'
          );
        });
      } else {
      ...
      if (display === 'grid') {
        it('should pass product link template to #linkContent', async () => {
          ...
          expect(atomicProductElements?.[0].linkContent?.querySelector('a')?.href).toBe(
            'https://example.com/123'
          );
          expect(atomicProductElements?.[1].linkContent?.querySelector('a')?.href).toBe(
        ...
        it('should pass empty link template to #linkContent', async () => {
          ...
          expect(atomicProductElements?.[0].linkContent).toBe(mockEmptyLinkTemplate);
          expect(atomicProductElements?.[1].linkContent).toBe(mockEmptyLinkTemplate);
        });
      }
      ...
      it('should pass correct #product', async () => {
        ...
        const atomicProductElement = element.shadowRoot?.querySelectorAll('atomic-product');
