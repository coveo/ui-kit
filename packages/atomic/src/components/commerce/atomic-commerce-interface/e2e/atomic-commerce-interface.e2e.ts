...
test.describe('when initializing the interface', () => {
  test.describe("when the access token isn't a JWT", () => {});
  test.describe('when the access token is JWT', () => {});
});
test.describe('when a query is performed automatically', () => {
  ...
  test.describe('when a language is provided', () => {
    test('should set the language of the interface', async ({
      ...
      await expect(commerceInterface.interface()).toContainText('Résultats');
    });
    ...
    test('should default to english when an invalid language is selected', async ({
      ...
      await expect(commerceInterface.interface()).toContainText('Results');
    });
    ...
    test('should default back to the non region locale (e.g., "es-ES" to "es")', async ({
      ...
      await expect(commerceInterface.interface()).toContainText('Resultados');
    });
