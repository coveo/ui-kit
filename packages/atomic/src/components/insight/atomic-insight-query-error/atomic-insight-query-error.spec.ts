import {buildQueryError} from '@coveo/headless/insight';
...
import './atomic-insight-query-error';
import type {QueryError} from '@coveo/headless/insight';
import {userEvent} from '@vitest/browser/context';
import {getAriaMessageFromErrorType} from '../../common/query-error/utils';
import type {AtomicInsightQueryError} from './atomic-insight-query-error';

vi.mock('@coveo/headless/insight', {spy: true});
vi.mock('../../common/query-error/utils', {spy: true});

// Helper to build fake query error controller
const buildFakeQueryError = (
  ...
  const state = {...defaultState, ...(implementation.state || {})};

  const defaultImplementation: Partial<QueryError> = {
    subscribe: vi.fn((callback: () => void) => {
      // Call the callback immediately to set initial state
      callback();
...
describe('atomic-insight-query-error', () => {
  ...
  const renderQueryError = async ({
    ...
    const fakeQueryError = buildFakeQueryError({
      state: {
        hasError: !!error,
        error,
      },
    ...
    vi.mocked(buildQueryError).mockReturnValue(fakeQueryError);

    const {element} =
      await renderInAtomicInsightInterface<AtomicInsightQueryError>({
        // eslint-disable-next-line lit/no-unknown-tag-in-template
  ...
  it('should initialize with query error controller', async () => {
    ...
    expect(buildQueryError).toHaveBeenCalledWith(mockedEngine);
  });
  ...
  it('should call getAriaMessageFromErrorType with the correct parameters', async () => {
    ...
    expect(getAriaMessageFromErrorType).toHaveBeenCalledWith(
      expect.anything(),
      mockedEngine.state.configuration.organizationId,
      expect.any(String),
      mockError.type
    );
  });
  ...
  describe('when the error type is "Disconnected"', () => {
    ...
    it('should display the correct description', () => {
      expect(locators.description).toHaveTextContent(
        "Your query couldn't be sent to the following URL:"
      );
    });
  ...
  describe('when the error type is "NoEndpointsException"', () => {
    ...
    it('should display the correct title', () => {
      expect(locators.title).toHaveTextContent(
        `Your organization ${mockedEngine.state.configuration.organizationId} has no available content.`
      );
    });
  ...
  describe('when the error type is "InvalidTokenException"', () => {
    ...
    it('should display the correct title', () => {
      expect(locators.title).toHaveTextContent(
        `Your organization ${mockedEngine.state.configuration.organizationId} cannot be accessed.`
      );
    });
  ...
  describe('when the error type is "OrganizationIsPausedException"', () => {
    ...
    it('should display the correct title', () => {
      expect(locators.title).toHaveTextContent(
        `Your organization ${mockedEngine.state.configuration.organizationId} is paused due to inactivity and search is currently unavailable.`
      );
    });
