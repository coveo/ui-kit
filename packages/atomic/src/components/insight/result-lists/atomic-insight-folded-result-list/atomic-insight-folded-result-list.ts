...
import {
  ...
  type ResultListProps as InsightResultListProps,
...
/**
 * The `atomic-insight-folded-result-list` component is responsible for displaying folded query results in an Insight interface, by applying one or more result templates for up to three layers (that is, to the result, child, and grandchild).
 *
 * @internal
 ...
 */
@customElement('atomic-insight-folded-result-list')
@bindings()
@withTailwindStyles
  ...
  implements InitializableComponent<InsightBindings>
{
  ...
  private handleResolveFoldedResultList = (
    event: FoldedItemListContextEvent<InsightFoldedResultList>
  ) => {
  ...
  private initFolding(
    props: InsightResultListProps = {options: {}}
  ...
  private getContent(collection: InsightFoldedCollection) {
    const result = extractUnfoldedItem(collection);
    return this.resultTemplateProvider.getTemplateContent(result);
  }

  private getInteractiveResult(collection: InsightFoldedCollection) {
    const result = extractUnfoldedItem(collection);
  ...
  private getResultId(collection: InsightFoldedCollection) {
    const result = extractUnfoldedItem(collection);
