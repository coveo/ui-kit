...
/**
 * A [StencilJS property decorator](https://stenciljs.com/) to be used for folded result template components.
 * This allows the Stencil component to fetch the current folded result from its rendered parent, the `atomic-result` component.
 *
 *
 * @example
 ...
 * For more information and examples, view the "Utilities" section of the readme.
 */
export function FoldedResultContext() {
  return (component: ComponentInterface, resultVariable: string) => {
    const {connectedCallback, render} = component;
    component.connectedCallback = function () {
      const element = getElement(this);
      const event = buildCustomEvent(
        foldedResultContextEventName,
        (result: Result) => {
          this[resultVariable] = result;
        }
      );
...
type FoldedResultContextEventHandler = (result: FoldedResult) => void;
export type FoldedResultContextEvent =
  CustomEvent<FoldedResultContextEventHandler>;
const foldedResultContextEventName = 'atomic/resolveFoldedResult';
 ...
 * @returns A promise that resolves on initialization of the parent "atomic-result" element, or rejects when there is no parent "atomic-result" element.
 */
export const foldedResultContext = (element: Element) =>
  new Promise<FoldedResult>((resolve, reject) => {
    const event = buildCustomEvent<FoldedResultContextEventHandler>(
      foldedResultContextEventName,
      (result: FoldedResult) => resolve(result)
    );
    element.dispatchEvent(event);
