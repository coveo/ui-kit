...
Installing Atomic in your Vue project is very similar to [installing Atomic](https://static.cloud.coveo.com/atomic/v3/storybook/index.html?path=/docs/usage-introduction--docs#install-atomic) outside of Vue.
However, there are some extra steps if you choose to [install Atomic via npm](https://static.cloud.coveo.com/atomic/v3/storybook/index.html?path=/docs/usage-in-a-framework-usage-in-a-vue-project--docs#npm).
...
The Atomic package allows you to customize your components by defining CSS variables in your own stylesheet (see [Themes and visual customization](https://static.cloud.coveo.com/atomic/v3/storybook/index.html?path=/docs/usage-themes-and-visual-customization--docs)).
...
1. Initialization (see [Use components to create a search interface](https://static.cloud.coveo.com/atomic/v3/storybook/index.html?path=/docs/usage-introduction--docs#use-components-to-create-a-search-interface)).
...
This poses an obstacle when working with the `<atomic-result-template>` component, which expects a native `<template>` that it will then use to create each result element (see [Defining a result template](https://static.cloud.coveo.com/atomic/v3/storybook/index.html?path=/docs/usage-display-results-introduction--docs#defining-a-result-template)).
