...
export namespace Components {
    ...
    interface AtomicInsightResultTemplate {
        ...
        /**
          * The field that, when defined on a result item, would allow the template to be applied.  For example, a template with the following attribute only applies to result items whose `filetype` and `sourcetype` fields are defined: `is-defined="filetype,sourcetype"`
         */
        "isDefined"?: string;
        /**
          * The field that, when defined on a result item, would prevent the template from being applied.  For example, a template with the following attribute only applies to result items whose `filetype` and `sourcetype` fields are NOT defined: `is-not-defined="filetype,sourcetype"`
         */
        "isNotDefined"?: string;
    }
