...
describe('UnifiedRuntime', () => {
  ...
  describe('event dispatch', () => {
    ...
    it('stores STATE_SNAPSHOT events as opaque agent state', async () => {
      const config = createMockConfig();
      const engine = createMockEngine();
      const snapshot = {filters: ['in-stock']};
      setupSuccessfulStream([
        {type: 'STATE_SNAPSHOT', snapshot} as NormalizedStreamEvent,
        {type: 'turn_complete'} as NormalizedStreamEvent,
      ]);
  ...
  describe('a2ui-surface dispatch', () => {
    ...
    it('non-a2ui-surface ACTIVITY_SNAPSHOT only appends an opaque activity', async () => {
      const config = createMockConfig();
      const engine = createMockEngine();
    ...
    it('a2ui-surface ACTIVITY_SNAPSHOT appends an activity and triggers hydration', async () => {
      const config = createMockConfig();
      const engine = createMockEngine();
