...
interface BrowserContext {
  // biome-ignore lint/suspicious/noExplicitAny: dynamically imported playwright Browser type
  browser: any;
  // biome-ignore lint/suspicious/noExplicitAny: dynamically imported playwright Page type
  page: any;
}
...
interface LLMClientWrapper {
  // biome-ignore lint/suspicious/noExplicitAny: OpenAI client type from dynamic import
...
async function callLLMWithRetry(
  ...
  config: Config,
  maxRetries: number = 5
  // biome-ignore lint/suspicious/noExplicitAny: OpenAI response type from dynamic import
): Promise<any> {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // biome-ignore lint/suspicious/noExplicitAny: catch-all error handler for unknown error types
    } catch (error: any) {
      lastError = error;
...
function validateLLMResponse(
  // biome-ignore lint/suspicious/noExplicitAny: validates untyped JSON response from LLM
  parsed: any,
  expectedKeys: string[]
