...
import {ALL_AI_CRITERIA, SURFACE_PREFIXES} from '../shared/constants.js';
...
export async function main(): Promise<void> {
  ...
  // Concurrency pool: process stories across N browser pages
  let cursor = 0;
  async function worker(page: PlaywrightPage): Promise<void> {
    while (!abortError) {
      const next = cursor++;
      if (next >= toProcess.length) return;

      const {index, story} = toProcess[next];
  ...
  // Launch N workers, one per browser page
