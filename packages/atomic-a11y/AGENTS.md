...
| Add/modify tests | `src/__tests__/vitest-a11y-reporter.spec.ts` | Single test file covers utils + integration |
...
|--------|------|----------|------|
| `VitestA11yReporter` | class | `src/reporter/vitest-a11y-reporter.ts` | Core: implements Vitest `Reporter`, aggregates results, writes JSON |
...
```
Storybook Test Run
  → Vitest calls onTestResult() per test case
  ...
  → onFinished(): build components[] + criteria[] + summary → write JSON
  → Output: a11y-report.json (+ shard variant if parallel CI)
```
...
- **Shard support**: In CI with `--shard=N/M` or env vars (`VITEST_SHARD`, `CI_NODE_INDEX`), reporter writes both `a11y-report.json` and `a11y-report.shard-N.json`
- **Component detection**: Relies on `atomic-*` naming convention in module paths and story IDs
- **Category detection**: Extracted from path segments (`commerce/`, `search/`, `insight/`, etc.) or story ID prefixes
